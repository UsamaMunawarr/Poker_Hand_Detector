from cv2 import sort
def findPokerHand(hand):

    ranks = []
    suits = []
    possibleRanks = []
    ###################
    # Convert the hand into ranks and suits
    # Example: hand = ["KH", "AH", "QH", "JH", "10H"]
    # ranks = [13, 14, 12, 11, 10]
    # suits = ["H", "H", "H", "H", "H"]
    # where K=13, A=14, Q=12, J=11, 10=10
    ###################
    for card in hand:
        if len(card) == 2:
            rank = card[0]
            suit = card[1]
        else:
            rank = card[0:2]
            suit = card[2]
        if rank == "A": 
            rank = 14
        elif rank == "K": 
            rank = 13
        elif rank == "Q": 
            rank = 12
        elif rank == "J":
            rank = 11

        ranks.append(int(rank))
        suits.append(suit)
    sortedRanks = sorted(ranks)
    ###########################
    # Royal Flush and Straight Flush and Flush
    ################################
    if suits.count(suits[0]) == 5: # check for flush
        if 14 in sortedRanks and 13 in sortedRanks and 12 in sortedRanks and 11 in sortedRanks and 10 in sortedRanks:
            possibleRanks.append(10)
        #########
        # Straight Flush
        # 10 11 12 13 14
        # 11 == 10 + 1
        ############
        elif all(sortedRanks[i] == sortedRanks[i-1] + 1 for i in range(1, len(sortedRanks))):
            possibleRanks.append(9)
        #########
        # Flush
        ############
        else:
            possibleRanks.append(6)
    ############
    # Straight
    # 10 11 12 13 14
    # 11 == 10 + 1
    ###########
    if all(sortedRanks[i] == sortedRanks[i-1] + 1 for i in range(1, len(sortedRanks))):
        possibleRanks.append(5)
    ###########
    handUniqueValues = list(set(sortedRanks))
    #############
    # Four of a Kind
    # 3 3 3 3 5   -- set --- 3 5 --- unique values = 2 --- Four of a kind
    # 3 3 3 5 5   -- set -- 3 5 ---- unique values = 2 --- Full House
    ##############
    if len(handUniqueValues) == 2:
        for val in handUniqueValues:
            if sortedRanks.count(val) == 4:
                possibleRanks.append(8)
             ##############
             # Full House
             ###############   
            if sortedRanks.count(val) == 3: 
                possibleRanks.append(7)

    ##############
    # Three of a Kind   
    # 5 5 5 6 7 -- set -- 5 6 7 --- unique values = 3   -- three of a kind
    # 8 8 7 7 2 -- set -- 8 7 2 --- unique values = 3   -- two pair
    ##############
    if len(handUniqueValues) == 3:
        for val in handUniqueValues:
            if sortedRanks.count(val) == 3:
                possibleRanks.append(4)
            ################
            # Two Pair
            ################
            if sortedRanks.count(val) == 2:
                possibleRanks.append(3)

    ##############
    # One Pair
    # 5 5 3 6 7 -- set -- 5 3 6 7 - unique values = 4 -- Pair
    #############
    if len(handUniqueValues) == 4:
        possibleRanks.append(2)
    ##############
    # High Card
    # 5 3 6 7 8 -- set -- 5 3 6 7 8 - unique values = 5 -- High Card
    ##############
    if not possibleRanks:
        possibleRanks.append(1)  #
    ###################
    # Map the possible ranks to their corresponding poker hand names
    ###################
    # print("Possible Ranks:", possibleRanks)
    pokerHandRanks = {10: "Royal Flush", 9: "Straight Flush", 8: "Four of a Kind", 7: "Full House", 6: "Flush", 
                      5: "Straight", 4: "Three of a Kind", 3: "Two Pair", 2: "One Pair", 1: "High Card"}
    ###############
    # Get the best possible hand
    ###################
    output = pokerHandRanks[max(possibleRanks)]
    print(hand, output)
    return output
########################
# Example usage
########################
if __name__ == "__main__":
    findPokerHand(["KH", "AH", "QH", "JH", "10H"])  # Example input for a Royal Flush
    findPokerHand(["QC", "JC", "10C", "9C", "8C"]) # Example input for a Straight Flush
    findPokerHand(["5C", "5S", "5H", "5D", "QH"])  # Example input for a Four of a Kind
    findPokerHand(["2H", "2D", "2S", "10H", "10C"])  # Example input for a Full House
    findPokerHand(["2D", "KD", "7D", "6D", "5D"])  # Example input for a Flush
    findPokerHand(["JC", "10H", "9C", "8C", "7D"])  # Example input for a Straight
    findPokerHand(["10H", "10C", "10D", "2D", "5S"])  # Example input for a Three of a Kind
    findPokerHand(["KD", "KH", "5C", "5S", "6D"])  # Example input for a Two Pair
    findPokerHand(["2D", "2S", "9C", "KD", "10C"])  # Example input for a One Pair
    findPokerHand(["KD", "5H", "2D", "10C", "JH"])  # Example input for a High Card

